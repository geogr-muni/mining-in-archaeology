...
out_file = 'out/train_1.csv'
head_no = 0
no_iters = 10

table_dist <- function(a) {
  tab = as.matrix(dist(a))
  return (round(tab / max(tab), 5))
}
...
for (comp in colnames(c)) {
  ...
  out <- add_column(out, paste(comp, 'dist', sep=""), dist )
  out <- add_column(out, paste(comp, 'dist_w', sep=""), dist_w)
  print(if (dist_w > dist) 0 else 1)
  out <- add_column(out, paste(comp, '_predicted', sep=""), if (dist_w > dist) 0 else 1)
}

write.csv(out, file="out2.csv")
write.csv(out[out[,'test'] == TRUE,], file="out_test.csv")
